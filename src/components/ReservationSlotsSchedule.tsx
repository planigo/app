...
const ReservationSlotsSchedule = ({
  reservationSlots,
}: ReservationSlotsScheduleProps) => {
        ...
        >
          ...
          >
            <p style={{ textAlign: "center" }}>
              {dayjs(r.date).format(TEXTUAL_DATE_FORMAT)}
            </p>
            {r.slots &&
              r.slots.map((s) => (
                ...
                <Button
                  ...
                  color="secondary"
                  variant="contained"
                  key={`${s.start}-${s.end}-${r.date}`}
                >
                  {dayjs(getReservationDateHour(r.date, s.start)).format(
                    HOUR_MINUTES_FORMAT
                  )}
                </Button>
                // </Tooltip>
...
export default dynamic(() => Promise.resolve(ReservationSlotsSchedule), {
  ssr: false,
});
